...
The MCP tool catalog also carries conservative side-effect annotations. `dsh_status`, `dsh_history`, `dsh_list`, and `dsh_goal_status` are marked read-only/idempotent; session/goal control tools are explicitly marked mutating and potentially consequential so clients can apply read-only filters or approval policies without guessing from tool names.
