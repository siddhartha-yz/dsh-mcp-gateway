...
The repository's `browser_session` plugin is a reviewed runtime extension for persistent Chromium interaction. It exposes `open/list/status/snapshot/act/script/close` without an internal model loop, scheduler, or autonomous browser task. Browser sessions are exact-Agent scoped; page, cookie, DOM, and in-memory browser state persist across tool calls while the session is alive. Playwright supplies protocol control only. DSH still computes the `sandbox.confine(...)` argv and therefore owns file-effect policy; a narrow local `dsh-browser-worker` launches only the pinned Chromium (or the pinned DSH Landlock launcher followed by that Chromium) under a dedicated AppArmor `userns` profile, then runs with `NoNewPrivs=1`. The worker accepts spawn requests only from the live `dsh-web-host.service` MainPID via Unix `SO_PEERCRED`, contains no shell/model/scheduler, and owns browser process-tree/profile cleanup. The runtime lock pins `playwright-core` and the small Koffi binding used for peer-credential lookup, while deployment installs the matching Chromium build reproducibly.
