...
The repository's `task_state` plugin is one explicitly reviewed extra capability. It stores compact engineering-task records through DSH's native `storageDomain`: title/goal/workspace, current summary and plan, candidate next steps, references, status, revision, and a bounded checkpoint history. Its `resume` action only changes a paused record back to `active`; it does not resume a process, invoke a model, choose an action, or continue work. Existing-task mutations require the latest `if_revision`, so stale conversations fail closed instead of silently overwriting newer checkpoints.

The repository's `shell_session` plugin is another reviewed extra capability. It exposes `open/list/status/send/read/signal/close` over an isolated DSH `TerminalSessionService` plus the native `terminal-bash` backend. The adapter contains no PTY, process, or sandbox implementation: DSH retains exact-Agent ownership, persistent cwd/environment/process context, bounded scrollback/output, foreground signaling, timeouts, process-tree cleanup, and the shared sandbox policy. The existing one-shot `bash` tool remains available for simpler commands.

The repository's `browser_session` plugin is a reviewed runtime extension for persistent Chromium interaction. It exposes `open/list/status/snapshot/act/script/close` without an internal model loop, scheduler, or autonomous browser task. Browser sessions are exact-Agent scoped; page, cookie, DOM, and in-memory browser state persist across tool calls while the session is alive. Playwright supplies protocol control only. DSH remains the file-effect policy authority: the plugin resolves the current DSH `sandboxPolicy` and asks the configured DSH sandbox provider to validate it fail-closed. For `workspace-write`, Chromium then uses the reviewed `@deepseek-ai/dsh-sandbox-local` 0.1.2-rc.1 bwrap mount profile (`/` read-only, private `/proc` and `/tmp`, workspace bind-mounted writable) because DSH's Landlock fallback intentionally blocks the `/proc/*/uid_map` writes Chromium needs for its own user-namespace sandbox. A narrow local `dsh-browser-worker` launches only that exact bwrap+Chromium argv (or pinned Chromium directly for `danger-full-access`) under a dedicated AppArmor `userns` profile and `NoNewPrivs=1`. The worker accepts spawn requests only from the live `dsh-web-host.service` MainPID via Unix `SO_PEERCRED`, contains no shell/model/scheduler, and owns browser process-tree/profile cleanup. The runtime lock pins `playwright-core` and the small Koffi binding used for peer-credential lookup, while deployment installs the matching Chromium build reproducibly.
