...
The bridge then reads `ctx.tools.schemas(agent)` and executes calls with `ctx.tools.execute({ ..., agent })`; SkillRegistry discovery/loading receives the same agent scope. The public OAuth/MCP gateway talks only to that loopback bridge. Its stable meta-tools query this live scoped catalog on demand, while compatible schemas may additionally be projected as first-class MCP tools when the client refreshes its tool surface. DSH therefore remains authoritative for preset composition, scoped registrations, restrictions, guards, jobs/filesystem/session ownership, skills, and community extensions.

```text
community DSH plugin / preset
        -> DSH agent-scoped ToolRuntime + SkillRegistry
        -> loopback bridge
        -> OAuth/MCP adapter
        -> ChatGPT Web
```

The correctness path is catalog-driven rather than wrapper-driven: the fixed `dsh_tool_catalog`/`dsh_tool_call` pair reads and executes against the current preset-scoped DSH ToolRuntime view at call time. Therefore a newly loaded compatible DSH tool remains usable even if ChatGPT keeps the MCP tool list it approved earlier and never refreshes it. First-class projection is an optional UX layer: each MCP `tools/list` also reads the live DSH view, the DSH plugin increments a process-local revision from native `tools/change` events, and the gateway publishes MCP `tools/list_changed` through the SDK subscription bus. Clients that honor those notifications can gain direct first-class tool entries while staying connected; clients that do not still retain full generic access through the stable meta-tools. The gateway never mirrors or owns the DSH catalog. If a host has no AgentRegistry/preset service, the bridge can still expose the global DSH view, but the shipped DSH deployment uses the scoped path. A real rc6 smoke exercised preset-scoped `bash` discovery and execution without any model-provider API key.
