...
The primary bridge is deliberately small and stays inside DSH's native capability model. For discovery, a DSH-resident Cordis plugin asks `agentPresets.standingKeyFor()` for the default preset's standing scope. DSH documents this host-reader seam as composing the preset without starting an Agent, Session, or turn. ChatGPT remains the reasoning agent.

Some DSH-native tools, notably `read_image`, deliberately gate execution on the calling Agent's exact routed model metadata, and rc6 `ToolRuntime.execute()` accepts an Agent rather than an arbitrary standing scope. The bridge therefore creates an execution-only capability Agent lazily on the first tool call. Its durable helper id hashes the workspace cwd, preset id, resolved composition path, and the composition file's `mtimeMs`/size stamp. Those are the public path plus the same file-stamp dimensions rc6 uses to advance a standing preset generation. This both keeps session ids unique across project directories and prevents a changed default preset or hot-edited composition from reusing an Agent composed for an older generation. `presets.mount(agentCtx, presetId)` is explicit, and setup checks the actually returned preset path plus a post-mount stamp; a concurrent composition change therefore rolls creation back instead of publishing a mismatched helper. Stable generations resume across DSH restarts rather than minting a new durable session per boot. The Agent's local metadata-only route declares `text` and `image` input, has no credential, and hard-fails inference. This gives DSH's existing modality and policy gates truthful capability metadata without introducing another reasoning path.

The bridge reads `ctx.tools.schemas(scope)` and SkillRegistry through the preset standing scope, so catalog and skill operations do not create helper sessions. Calls execute with `ctx.tools.execute({ ..., agent })` through the lazy execution identity, preserving DSH's guarded pipeline. The public OAuth/MCP gateway talks only to that loopback bridge. Its stable meta-tools query this live scoped catalog on demand. In the shipped/default `meta-only` mode, compatible DSH schemas are intentionally *not* projected into ChatGPT's MCP tool list. DSH therefore remains authoritative for preset composition, scoped registrations, restrictions, guards, jobs/filesystem/session ownership, skills, and community extensions.

```text
community DSH plugin / preset
        -> DSH preset standing scope (discovery)
        -> DSH Agent-scoped ToolRuntime (execution)
        -> loopback bridge
        -> OAuth/MCP adapter
        -> ChatGPT Web
```

The correctness path is catalog-driven rather than wrapper-driven: the fixed `dsh_tool_catalog`/`dsh_tool_call` pair reads and executes against the current preset-scoped DSH ToolRuntime view at call time. Therefore a newly loaded compatible DSH tool remains usable while ChatGPT's MCP schema stays fixed. Meta-only mode serves exactly the stable gateway tools and removes the modern `subscriptions/listen` handler so the MCP handshake does not advertise `tools.listChanged`; a client has no first-class refresh channel to rely on. First-class projection remains an explicit optional UX layer selected with `--tool-surface projected`: only then does each MCP `tools/list` read the live DSH view and the gateway publish DSH `tools/change` revisions through MCP subscriptions. The bridge revision token includes a per-plugin-instance id as well as the numeric tool revision, so a DSH/bridge restart cannot reset the counter to an apparently unchanged value and suppress the required invalidation. The gateway never mirrors or owns the DSH catalog. If a host has no AgentRegistry/preset service, the bridge can still expose the global DSH view, but the shipped DSH deployment uses the scoped path. A real rc6 smoke exercised preset-scoped `bash` discovery and execution without any model-provider API key.
