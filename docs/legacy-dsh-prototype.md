...
Production deployments do not need the development tools. The package keeps `mcp>=2,<3` as its compatibility range, while [`deploy/server-constraints.txt`](deploy/server-constraints.txt) records the exact Python server dependency graph currently exercised by the deployment CI. The DSH side is separately reproducible: [`deploy/dsh-runtime/package.json`](deploy/dsh-runtime/package.json) pins `@deepseek-ai/dsh@0.1.0-rc.6`, its checked-in npm lock records the 587 resolved transitive packages with sha512 integrity metadata, and the example service uses a self-contained Node `24.19.0` runtime. `scripts/verify-server-constraints.py` and `scripts/verify-dsh-runtime-lock.py` enforce those two deployment snapshots before a known-good rebuild. A clean Linux x86_64/glibc reconstruction from that npm lock has also been exercised from an empty directory: 530 runtime packages installed with npm `11.17.0`, the rc6 Web Host/Agent initialized, a model-driven `bash` call executed successfully, and `node-pty` spawned a PTY. The deployment command pins the npm registry explicitly instead of inheriting host-global mirror configuration.
