...
Current experiment evidence: the official DSH Web client-plugin seam is proven, and B1 automatic outbound is proven end-to-end in a real ChatGPT conversation: a DSH-queued message crossed the official MCP App `ui/message` path and started the next model turn with no ChatGPT-side click, while the companion iframe remained alive afterward. The official Apps surface does not expose the subsequent assistant transcript or a reliable assistant-turn-completed event, so P6 has moved to B2 for receive-side observation only. A minimal read-only browser extension is now prepared to report semantic ChatGPT generation/assistant/completion signals into the existing DSH bridge; B1 remains the send path, and live B2 acceptance is still pending.
