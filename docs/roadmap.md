...
### P6 — DSH GUI control surface for ChatGPT Web — COMPLETE

P6 keeps the official DSH Web GUI as the control surface while the real ChatGPT Web conversation remains the only primary reasoning agent. B1 sends queued follow-ups through the official MCP App `ui/message` path; B2 is a minimal read-only Firefox observer for assistant-turn lifecycle; the controller performs only mechanical continuation gating and never plans, reasons, or mutates `task_state`.

Acceptance result: the isolated real ChatGPT Web runner passed the canonical three-part contract. **Transport success** proved the intended message crossed B1 and the bound observer reported the matching lifecycle. **Progress success** proved each expected turn advanced `task_state.revision` while preserving the required task status. **Continuation success** proved the seed was excluded from counting, exactly three continuations were delivered, and the controller stopped mechanically with `continuationCount=3`, `stopReason=max_continuations_reached`, and an empty queue. Dedicated regressions keep transport, progress, and continuation failures independently diagnosable. The GUI/bridge remains a control and transport surface, not another model loop or harness.

Detailed design, fallback ladder, and acceptance evidence: [`experiments/dsh-gui-chatgpt-web.md`](experiments/dsh-gui-chatgpt-web.md).
