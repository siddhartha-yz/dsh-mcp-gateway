...
Current experiment evidence: the official DSH Web client-plugin seam is proven, and B1 automatic outbound is proven end-to-end in a real ChatGPT conversation: a DSH-queued message crossed the official MCP App `ui/message` path and started the next model turn with no ChatGPT-side click, while the companion iframe remained alive afterward. Because the official Apps surface does not expose the subsequent assistant transcript or reliable turn completion, B2 adds a minimal read-only browser observer. A version-4 mechanical controller is also prepared: after an observed completed turn it can enqueue exactly one B1 continuation only when the selected `task_state` remains active, its revision advanced during that turn, transport health is good, the conversation matches, and the turn has not already been continued. It stops on completed/paused/degraded/ambiguous state. Protocol tests prove repeated three-turn chaining and completion stop; live B2 plus 3-5 real-turn acceptance remains pending.
