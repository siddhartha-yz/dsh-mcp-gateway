...
### P6 — DSH GUI control surface for ChatGPT Web — EXPERIMENTAL / NOT RELIABLE FOR UNATTENDED USE

P6 explored a DSH Web control surface for mechanically continuing work in the real ChatGPT Web conversation. B1 sends queued follow-ups through the official MCP App `ui/message` path; B2 is a read-only Firefox DOM observer for assistant-turn lifecycle; the controller gates continuation on observer health, conversation binding, queue state, and `task_state.revision`. It contains no model loop or autonomous reasoning.

Evidence is mixed. An isolated exact-three run passed the transport/progress/continuation contract: the seed was excluded, three continuations advanced task revisions, and the controller stopped with an empty queue. A later attempt to start a real 100-minute repository-audit soak did **not** reproduce unattended operation reliably: the observer required manual page/extension intervention, the production control path remained sensitive to browser lifecycle and UI state, the controller was easy to arm with an unsuitable default continuation cap, and the expected automatic continuation did not become a dependable hands-off workflow. Exact-three therefore remains useful experimental evidence, not proof of production reliability.

P6 is source-tracked for future experiments, but it is disabled from the default production DSH overlay and the production gateway no longer enables the experimental companion flag. `task_state` remains the supported cross-conversation resume mechanism; long unattended ChatGPT Web execution is not a supported production capability.

Detailed design, historical acceptance evidence, and postmortem: [`experiments/dsh-gui-chatgpt-web.md`](experiments/dsh-gui-chatgpt-web.md).

### P7 — DSH remote workers — COMPLETE

P7 was added only after P6 created a concrete need to inspect the user's local Firefox host. It implements DSH-native one-time enrollment plus outbound-polling remote workers; remote machines do not install DSH, ChatGPT remains the reasoning agent, and DSH remains the execution/runtime authority.

Acceptance result: the integrated live host exposes `remote_machine` and `remote_exec`; the user's `local-ubuntu` worker is persistently enrolled and online. Live remote execution has successfully run shell commands and filesystem operations on the Ubuntu desktop, proving the second execution environment is usable through the same DSH control plane. P7 v1 intentionally stays narrow: remote administration plus bounded shell/filesystem execution, without adding another agent loop or pre-emptively cloning persistent terminal/browser routing.
