...
Implementation checkpoint: P5 is implemented as a repository-local `browser_session` ToolRuntime plugin rather than adopting `dsh-browseruse` wholesale. The reviewed third-party plugin was rejected as the production surface because it carries its own LLM-driven `browser_task`, scheduler, and process-global browser singleton. The P5 plugin instead keeps ChatGPT as the only reasoning agent and exposes only open/list/status/snapshot/act/script/close. Each live browser session is owned by the exact DSH Agent. DSH still computes the Chromium argv through the current `sandboxPolicy`/`sandbox.confine` path, but Ubuntu's AppArmor userns restriction cannot be acquired by a Chromium child after the main DSH Host already has `NoNewPrivileges=1`. The production design therefore uses a narrow `dsh-browser-worker` systemd boundary: AppArmor attaches the dedicated `userns` profile before the worker sets `NoNewPrivs=1`; the worker then accepts spawn requests only from the live DSH Host MainPID using Unix `SO_PEERCRED`, permits only the pinned DSH Landlock launcher plus pinned Chromium (or pinned Chromium directly for danger-full-access), rejects `--no-sandbox`, and owns process/profile cleanup. Browser profiles live in the worker's `PrivateTmp`, which remains writable under DSH's workspace-write Landlock grant without exposing them to the Host's private `/tmp`. The runtime lock pins `playwright-core` 1.62.1 and Koffi 3.2.1, and host browser/AppArmor prerequisites remain an explicit root provisioning step. Local CI-equivalent gates currently pass 196 Python tests plus the JS/systemd/runtime-lock suites; production reprovision/redeploy and live browser-session acceptance are still required before P5 is complete.
